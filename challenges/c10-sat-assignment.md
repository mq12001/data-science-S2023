...
| `math_SAT` | SAT Score in Math      |
| `verb_SAT` | SAT Score in Verbal    |
| `comp_GPA` | Computer Science GPA   |
| `univ_GPA` | Overall University GPA |

- What information do we have about these students?
  - They all graduated and all had a BS in computer science
- What kinds of information *do we not have* about these students?
  - The rigor of their high schools/colleges, what courses they took,
    the number of times they took the SAT, whether this is a superscore
- Based on these missing variables, what possible effects could be
  present in the data that we would have *no way of detecting*?
  - Family’s income, other environmental advantages that would lead to a
    higher score on standardized tests, like the ability to take it
    multiple times
...
df_composite %>% 
  arrange(univ_GPA) %>% 
  ggplot() + 
  geom_point(aes(univ_GPA, high_GPA, color = both_SAT))
```
...
- What relationship do `univ_GPA` and `high_GPA` exhibit?
  - They are directly related - as one increases, so does the other, but
    more linearly than univ_GPA and both_SAT.
...
df_composite %>% 
  ggplot() + 
  geom_histogram(aes(both_SAT), bins = 40)
```
...
df_composite %>% 
  ggplot() + 
  geom_histogram(aes(high_GPA), bins = 40)
```
...
df_composite %>% 
  ggplot() + 
  geom_histogram(aes(univ_GPA), bins = 40)
```
...
- To what extent does `high_GPA` look like a normal distribution?
  - Looks mildly weighted to be higher, as opposed to a normal
    distribution (symmetrical)
- To what extent does `univ_GPA` look like a normal distribution?
  - Looks distinctly weighted to be higher, as opposed to a normal
    distribution (symmetrical)
...
- Which correlations are significantly nonzero?
  - Both, correlation with high school GPA is estimated to be .78 and
    correlation with SAT is estimated to be .68
- Which of `high_GPA` and `both_SAT` seems to be more strongly
  correlated with `univ_GPA`?
  - high_GPA
- How do the results here compare with the visual you created in q2?
  - Generates the same results: high_GPA is more strongly correlated
    with univ_GPA than both_SAT, but they are both correlated to some
    extent.
- Based on these results, what can we say about the predictive
  capabilities of both `high_GPA` and `both_SAT` to predict `univ_GPA`?
  - high_GPA is more predictive than both_SAT of univ_GPA, but they are
    both predictive to some extent.
...
corr_fun <- function(split) {
  ...
  df_tmp <-
    bind_rows(tibble(
      estimate = cor(univ_GPA, high_GPA),
      term = "cor"
    ))
}
...
df_q5 <-
  ...
  mutate(estimates = map(splits, corr_fun))
...
- How does your estimate from q5 compare with your estimate from q4?
  - They are almost exactly the same, .78
- How does your CI from q5 compare with your CI from q4?
  - Also almost exactly the same, .70 to .85
...
- What is the confidence interval on the coefficient of `both_SAT`? Is
  ...
  - The confidence interval is from .002 to .003, which isn’t
    significantly different from zero
- By itself, how well does `both_SAT` predict `univ_GPA`?
  - The confidence interval is fairly wide compared to the values it
    lies at, so it isn’t all that accurate of a predictor.
...
fit_both <- df_composite %>%
  lm(
    data = ., 
    formula = univ_GPA ~ high_GPA + both_SAT
  )
...
- What is the confidence interval on the coefficient of `both_SAT` when
  ...
  significantly different from zero?
  - -.0002 to .001, which is NOT significantly different from zero
- How do the hypothesis test results compare with the results in q6?
  - This test disproves the null hypothesis, while the q6 test does not.
...
- Between `both_SAT` and `high_GPA`, which single variable would you
  ...
  - I would pick high_GPA, because its reliability as a predictor was
    proven to be greater by multiple tests of correlation, and its
    confidence intervals were always smaller.
- Is `both_SAT` an effective predictor of `univ_GPA`? What specific
  pieces of evidence do you have in favor of `both_SAT` being effective?
  What specific pieces of evidence do you have against?
  - Both_SAT is a mildly but not extremely effective predictor of
    univ_GPA, as proved by its corr value being higher than .5, but
    lower than the corr value for high_GPA.
